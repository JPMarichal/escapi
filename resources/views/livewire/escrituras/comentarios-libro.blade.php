    ...
    <div class="max-w-4xl mx-auto px-4 py-8 bg-white relative border-l border-green-600/30 border-r">
        ...
        <div class="flex items-center justify-between mb-8">
            <h1 class="text-xl font-semibold text-green-700">
                Comentarios a {{ $libro->nombre }}
            </h1>
            <a href="{{ route('escrituras.libro', ['libro' => strtolower($libro->nombre)]) }}" 
        ...
        <div class="space-y-8">
                        ...
                        <div class="px-4 py-3 border-b border-gray-200">
                            <div class="flex items-center justify-between">
                                <h2 class="text-lg font-medium text-green-700">
                                    {{ $libro->nombre }}:{{ $versiculo->num_versiculo }}
                                </h2>
                                <button 
                                    x-data="{ open: false }"
                                    @click="open = !open"
            ...
            @empty
                <div class="text-center py-12">
                    <p class="text-gray-500">No hay comentarios en este libro.</p>
                </div>
            @endforelse
        </div>
    </div>
